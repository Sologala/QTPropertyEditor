...
TestWidget::TestWidget(QWidget *parent) : QWidget(parent), ui(new Ui::TestWidget)
{
    ...
    CDoubleProperty *d1 =
        new CDoubleProperty(dblhead, "SomeDouble1", "Double 1", 45.639, 0, -15.93, 1378789123232.327878273);
    ui->editor->add(d1);
    ...
    CListData list1;
    list1 << CListDataItem("Item 1", QIcon(":/Info")) << CListDataItem("Item 2")
          << CListDataItem("Item 3", QIcon(":/Open")) << CListDataItem("Item 4", QIcon(":/Remove"))
    ...
    CDateTimeProperty *datetime1 =
        new CDateTimeProperty(datehead, "DateTime1", "Date & Time", QDateTime(somedate, sometime));
